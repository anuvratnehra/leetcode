...
class Solution{
public:
    Node * create(int pre[], int start, int end){
        if(start>end) return NULL;
        //if(pre[start]<=min or pre[start]>=max) return NULL;
        Node *temp=newNode(pre[start]);
        int t=start;
        for(int i=start;i<=end;i++){
            if(pre[i]>pre[start]) break;
            t++;
        }
        temp->left=create(pre,start+1,t-1);
        temp->right=create(pre,t,end);
        return temp;
    }
